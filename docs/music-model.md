...
Este modelo define el vocabulario base. El prototipo actual fija un subconjunto practico para validar la experiencia: triadas diatonicas en los 7 modos sobre 12 raices cromaticas, inversiones de triada basicas y voicing cerrado automatico.
...
- La visualizacion de teclado resalta las notas exactas del voicing activo, no cada repeticion de la misma clase de altura entre C3-C6. La nota generadora (la tecla que el usuario presiono) lleva un acento visual propio (color coral/salmon) y siempre esta presente en el voicing cuando se dispara desde el teclado visual. Los tonos pasivos (notas del acorde que suenan porque pertenecen al voicing pero no fueron presionadas directamente) se resaltan en dorado.
- Los acordes disparados por **grados** usan por defecto un voicing ascendente desde el registro C4 en adelante; el control de registro permite mover esos disparos a C3, C4 o C5 sin cambiar la identidad abstracta del acorde. Si una triada cruza C, la nota superior sube a la siguiente octava. El control de inversion permite posicion fundamental, primera inversion y segunda inversion; cambia el bajo/voicing pero conserva identidad, grado, calidad y notas abstractas del acorde. Si el voicing excede el teclado C3-C6, se recorta a las notas visibles para playback y resaltado sin cambiar la identidad del acorde.
- Los acordes disparados por **teclado visual** usan un voicing anclado a la tecla presionada: el sistema rota las notas segun la inversion elegida y distribuye el acorde alrededor de la nota del teclado que el usuario toco. Por ejemplo, tocar C4 con primera inversion produce E3-G3-C4 (C4 como nota mas aguda) en lugar del voicing ascendente estandar E4-G4-C5. Esto garantiza que la tecla presionada siempre suene y actua como referencia visual y tactil.
...
- Las entradas son polifonicas: el usuario puede mantener presionadas multiples teclas o grados simultaneamente. Cada grado activo ilumina su boton correspondiente, asi que tocar varias notas del teclado que pertenezcan a distintos grados resalta todos esos grados a la vez.
